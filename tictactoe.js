...
//let move_expand_order=[4,0,1,2,3,5,6,7,8]; //6743
//let move_expand_order=[4,8,0,1,2,3,5,6,7]; //4954
//let move_expand_order=[4,8,7,0,1,2,3,5,6]; //5360
//let move_expand_order=[4,8,7,6,0,1,2,3,5]; //4759
//let move_expand_order=[4,8,7,0,6,1,2,3,5];  //4512
let move_expand_order=[4,8,7,0,6,2,1,3,5];    //4476
//let move_expand_order=[0,4,8,2,6,1,3,5,7]; //5594
...
}// THE CLOSING BRACKET FOR TICTACTOE_MINIMAX FUNCTION
...
for(var i = 0, j = 4; i<= 2; i = i+2, j = j-2){
    if(board[i] !== -1 && board[i] === board[i+j] && board[i+j] === board[i+2*j]){
      isTerminal = true;
    }
}
//no one won and board is filled, then it is also terminal state
...
function utility(board,player) {
  ...
  ***********************/
  ...
  //returns how many steps taken to reach terminal state
  for(var i = 0; i< board.length; i++){
      if(board[i] === -1){
          moves--;
      }
 ...
 //the game is draw, score of 0
 if(!someoneWon && moves === 9){
    return 0;
 }

 //player wins, assign the score. 9 moves = 1 point, 5 moves = 5 point
 /*   if(moves === 9){
      if(playerWins){
        score  = 1;
      }else{
...
function tictactoe_minimax_alphabeta(board,cpu_player,cur_player,alpha,beta) {
  ...
  ***********************/
  ...
  for(let move of move_expand_order) { //For each possible move (i.e., action)
    ...
    // What will my opponent do if I make this move?
    let results;
    results = tictactoe_minimax_alphabeta(new_board,cpu_player,1-cur_player, alpha, beta);
